...
- **Plan 3 should add Playwright e2e** for the core loop (boot against emulator → start trip → log items via the stepper → inline edit → delete-trip ConfirmDialog → screenshots for the no-emoji/Filipino visual pass). Decided to fold this into Plan 3 rather than a standalone Plan 2.5. Setup: `@playwright/test` + Chromium download + config that starts the preview server and boots the Firebase emulator. Web layer only — does NOT cover the Capacitor native shell.
```
